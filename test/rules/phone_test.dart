...
void main() {
  group('PhoneRule', () {
    ...
    test('passes on valid specific country formats', () {
      expect(const PhoneRule('Invalid', country: PhoneCountry.us).validate('+1 123 456 7890'), isNull);
      expect(const PhoneRule('Invalid', country: PhoneCountry.india).validate('+91 9876543210'), isNull);
      expect(const PhoneRule('Invalid', country: PhoneCountry.uk).validate('+447911123456'), isNull);
    });

    test('fails on invalid specific country formats', () {
      expect(const PhoneRule('Invalid', country: PhoneCountry.us).validate('+44 123 456 7890'), 'Invalid');
      expect(const PhoneRule('Invalid', country: PhoneCountry.india).validate('+91 123'), 'Invalid');
      expect(const PhoneRule('Invalid', country: PhoneCountry.uk).validate('011 123'), 'Invalid');
    });
  });
}
